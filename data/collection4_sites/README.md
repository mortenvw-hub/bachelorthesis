...
This collection contains supplementary information to various sites of different institutions listed in the location registry of the Head Office of the GBV Common Library Network (VZG). It contains information to locations, opening hours and contact details. The data sources, found in the `sources` subdirectory, consist of individual folders named with an ISIL and contain a `.ttl` file. They were provided by as `.txt` files by <https://github.com/gbv/libsites-config> and converted to the `ttl` format using various perl modules found in <https://github.com/gbv/libsites>. Both repositories are administrated by the GBV. For more in-depth information refer to the [Konfiguration der GBV-Standortverwaltung](https://verbundwiki.gbv.de/spaces/VZG/pages/35913745/Konfiguration+der+GBV-Standortverwaltung), German only.
